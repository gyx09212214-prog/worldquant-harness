...
DEFAULT_DB_PATH = Path("quantgpt.db")
...
def collect_submit_run_records(run_root: Path) -> list[dict[str, Any]]:
    ...
    for run_dir in sorted((p for p in run_root.iterdir() if p.is_dir()), key=lambda p: p.stat().st_mtime):
...
def mark_round_records(records: list[dict[str, Any]], round_start_after_alpha: str) -> None:
    ...
    for record in records:
        if record.get("alpha_id") == round_start_after_alpha:
            anchor_ts = max(anchor_ts, str(record.get("submitted_at") or ""))
    for record in records:
        active = str(record.get("platform_status") or "").upper() == "ACTIVE"
        after_anchor = bool(anchor_ts) and str(record.get("submitted_at") or "") > anchor_ts
