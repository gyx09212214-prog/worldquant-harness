"""Market data fetcher — worldquant-harness
...
try:
    from .wind_data import WindDataFetcher, fetch_wind_benchmark_returns, get_wind_universe, is_wind_enabled
except Exception:  # pragma: no cover - optional local adapter
    WindDataFetcher = None

    def is_wind_enabled():
        return False

    def get_wind_universe(*args, **kwargs):
        return []

    def fetch_wind_benchmark_returns(*args, **kwargs):
        return None
...
CACHE_ONLY = os.environ.get("WORLDQUANT_HARNESS_CACHE_ONLY", "").lower() in ("1", "true", "yes")
...
def _fetch_index_constituents(name: str, date: str | None = None) -> list[str]:
    ...
    if is_wind_enabled():
        wind_codes = get_wind_universe(name, date)
        if len(wind_codes) > 10:
            cache_path.write_text("\n".join(wind_codes))
            logger.info(f"[wind] Fetched {len(wind_codes)} constituents for {name}")
            return wind_codes
...
class MarketDataFetcher:
    ...
    def __init__(self, cache_dir: str | None = None):
        ...
        self.wind_fetcher = WindDataFetcher() if is_wind_enabled() and WindDataFetcher is not None else None
    ...
    def fetch_stocks(
        ...
        if to_fetch:
            if CACHE_ONLY:
                ...
                wind_fetched = set()
                if self.wind_fetcher is not None and self.wind_fetcher.available():
                    logger.info(f"[wind] Fetching {len(to_fetch)} stocks...")
                    try:
                        wind_df = self.wind_fetcher.fetch_stocks(to_fetch, start_date, end_date)
                    except Exception as e:
                        logger.warning(f"Wind fetch failed, falling back to public sources: {e}")
                        wind_df = None
                    if wind_df is not None and len(wind_df) > 0:
                        for bs_code, df in wind_df.groupby("stock_code", sort=False):
                            ...
                            if len(filtered) > 0:
                                all_data.append(filtered)
                            wind_fetched.add(self._normalize_stock_code(bs_code))

                to_fetch = [c for c in to_fetch if self._normalize_stock_code(c) not in wind_fetched]

                # Primary: baostock batch (free, no account needed)
                bs_fetched = set()
                if to_fetch and HAS_BAOSTOCK:
        ...
        if all_data:
            ...
            if (
                "amount" in result.columns
                and "volume" in result.columns
                and ("vwap" not in result.columns or result["vwap"].isna().all())
            ):
...
def fetch_benchmark_returns(
    ...
    wind_ret = fetch_wind_benchmark_returns(benchmark, start_date, end_date)
    if wind_ret is not None and len(wind_ret) > 1:
        pd.DataFrame({
            "trade_date": wind_ret.index,
            "close": np.nan,
            "daily_return": wind_ret.values,
        }).to_parquet(cache_path, index=False)
        logger.info(f"[wind] Benchmark {benchmark}: {len(wind_ret)} days")
        return wind_ret
