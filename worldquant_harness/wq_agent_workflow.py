...
from .wq_efficiency import annotate_candidate_identity, lifecycle_event
...
from .wq_policy_repair_planner import build_policy_repair_records, repair_candidate_concentration_risk
...
def presubmit_acceptance_gate(
    ...
    *,
    ...
    prod_result = str(row.get("prod_corr_result") or "").upper()
    daily_return_corr = safe_float(row.get("active_daily_return_corr_max") or row.get("active_daily_return_corr"))
...
def _simulation_setting_mismatches(requested: dict, actual: dict) -> list[dict]:
    ...
    for key in ("region", "universe", "delay", "decay", "neutralization", "truncation", "maxTrade", "maxPosition"):
        ...
        if _normalized_setting_value(key, requested_value) == _normalized_setting_value(key, actual_value):
            continue
        mismatches.append({
            "key": key,
...
def _normalized_setting_value(key: str, value: Any) -> Any:
    if value in (None, ""):
        return None
