"""Build a factor-domain map from worldquant-harness WQ artifacts.
...
class FactorMapConfig:
    ...
    title: str = "worldquant-harness 因子地图"
...
def build_field_summary(nodes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for node in nodes:
        for field_name in node.get("fields", []):
            by_field.setdefault(str(field_name), []).append(node)
    rows: list[dict[str, Any]] = []
    for field_name, field_nodes in by_field.items():
