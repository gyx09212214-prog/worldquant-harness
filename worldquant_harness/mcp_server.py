...
mcp = FastMCP(
    "worldquant_harness",
    instructions="worldquant-harness — A 股因子回测服务。先用 list_operators 了解可用算子，再用 run_backtest 执行回测。可用 score_factor 评分、diagnose_factor 诊断、run_anti_overfit 检测过拟合、run_rolling_validation 滚动验证。",
...
@mcp.tool()
def wq_harness_new(
    output_root: str = "reports/harness_runs/latest",
    topic: str = "WQ harness run",
    run_id: str = "harness-run",
    profile_name: str = "default",
) -> str:
    """创建一个 no-submit harness run 目录和初始契约文件。"""
    try:
        from .harness_runner import harness_new

        result = harness_new(output_root, topic=topic, run_id=run_id, profile_name=profile_name)
        return json.dumps(result, ensure_ascii=False, indent=2, default=str)
    except Exception as e:
        logger.error(f"wq_harness_new failed: {traceback.format_exc()}")
        return json.dumps({"ok": False, "no_submit": True, "error": str(e)}, ensure_ascii=False, indent=2)


@mcp.tool()
def wq_harness_run_presubmit(
    output_root: str = "reports/public_harness_eval",
    run_id: str = "public-harness-eval",
    mode: str = "public_demo",
    topic: str = "public harness eval",
) -> str:
    """运行 no-submit harness presubmit wrapper；默认执行 deterministic public demo/eval。"""
    try:
        from .harness_runner import harness_run_presubmit

        result = harness_run_presubmit(output_root, run_id=run_id, mode=mode, topic=topic)
        return json.dumps(result, ensure_ascii=False, indent=2, default=str)
    except Exception as e:
        logger.error(f"wq_harness_run_presubmit failed: {traceback.format_exc()}")
        return json.dumps({"ok": False, "no_submit": True, "error": str(e)}, ensure_ascii=False, indent=2)


@mcp.tool()
def wq_harness_evaluate(
    experiment: str,
    eval_id: str = "",
    output_dir: str = "",
    submit_run_dirs: list[str] | None = None,
) -> str:
    """评估已有 sandbox experiment，写 eval artifacts；只读提交记录，不会真实 submit。"""
    try:
        from .harness_runner import harness_evaluate

        result = harness_evaluate(
            experiment,
            eval_id=eval_id or None,
            output_dir=output_dir or None,
            submit_run_dirs=submit_run_dirs or [],
        )
        return json.dumps(result, ensure_ascii=False, indent=2, default=str)
    except Exception as e:
        logger.error(f"wq_harness_evaluate failed: {traceback.format_exc()}")
        return json.dumps({"ok": False, "no_submit": True, "error": str(e)}, ensure_ascii=False, indent=2)


@mcp.tool()
def wq_harness_evolve(
    experiment: str,
    eval_dir: str = "",
    output_root: str = "",
    ...
    """基于 eval artifact 生成下一轮 profile/experiment candidate；不会真实 submit。"""
    try:
        from .harness_runner import harness_evolve

        result = harness_evolve(
            experiment,
        ...
        logger.error(f"wq_harness_evolve failed: {traceback.format_exc()}")
        return json.dumps({"ok": False, "no_submit": True, "error": str(e)}, ensure_ascii=False, indent=2)


@mcp.tool()
def wq_harness_history_ingest(
    reports_dir: str = "reports",
    output_dir: str = "reports/harness_history_experience",
    no_platform: bool = True,
    local_file_limit: int = 0,
    event_limit: int = 0,
) -> str:
    """把本地历史记录沉淀为 canonical experience artifacts；默认不连平台。"""
    try:
        from .harness_runner import harness_history_ingest

        result = harness_history_ingest(
        ...
        logger.error(f"wq_harness_history_ingest failed: {traceback.format_exc()}")
        return json.dumps({"ok": False, "no_submit": True, "error": str(e)}, ensure_ascii=False, indent=2)


@mcp.tool()
def wq_harness_memory_maintain(
    memory_files: list[str] | None = None,
    output_dir: str = "reports/harness_memory_maintenance",
    compress_threshold: int = 50,
    absorb_threshold: int = 3,
) -> str:
    """生成 memory maintenance 和 memory_delta artifact；不修改原 memory 文件。"""
    try:
        from .harness_runner import harness_memory_maintain
        ...
        logger.error(f"wq_harness_memory_maintain failed: {traceback.format_exc()}")
        return json.dumps({"ok": False, "no_submit": True, "error": str(e)}, ensure_ascii=False, indent=2)


@mcp.tool()
def wq_harness_status(root: str = "reports/public_harness_eval") -> str:
    """读取 persisted harness run/eval 状态。"""
    try:
        from .harness_runner import harness_status

        result = harness_status(root)
        return json.dumps(result, ensure_ascii=False, indent=2, default=str)
    except Exception as e:
        logger.error(f"wq_harness_status failed: {traceback.format_exc()}")
        return json.dumps({"ok": False, "no_submit": True, "error": str(e)}, ensure_ascii=False, indent=2)
